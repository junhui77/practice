...
var personalHistoryGetData = function(){
	...
	$(".user-info-table1, .user-info-table2").find("input:not(input[name='file']), select").each(function(){ // 유동 테이블 두곳 루프 돌면서 데이터 획득
		...
		if(nowLoopId=="userEmail"){
			...
			var subData = nowLoopValue.split('@');	//	'@'를 구분으로 문자 분리하여 배열
			
			nowLoopValue = subData[0] + emailDomain; // 
			
			console.log(nowLoopValue);
			alert("이메일 테스트2222 ===> " + nowLoopValue);
		}
			...
			$.ajax({
				...
				success:function(data){
					...
					if(imgName==null){
						...
						//$("#thumbNail").attr("src","c:/upload/image/"+imgName);
