...
var btnEve = function(){	
	...
	// 리스트로가기 버튼
	$(".personalHistoryListAllBtn").unbind().click(function(){
		location.href = "/prac/"
	});
...
//	                alert("상세 주소를 입력해 주세요.")
	...
	$("#emailDomain").on("change",function(){
		...
		if(value=='직접입력'){
			...
			//(20221203_JJH)직접입력일 경우에 앞에 @붙여준다.
	...
	$("#marrySelect").on("change",function(){
		var value = $(this).val();
		if(value==""){
			$("#userMarry").attr("name","");
		}else{
			$("#userMarry").attr("name","userMarry")
		}
		$("input[name='userMarry']").val(value);
		console.log("이벤트 발생2");
	});
...
//상세페이지
// function detailLink(str){
// 	if(str=='' || str=='undefined'){
// 		location.href = context + "/detail";
// 	}else{
// 		$("#userIdx").val(str);
// 		$("#detailFrm").submit();
// 	}	
